...
- It installs dependencies, runs `npm run lint`, and runs `npm run build`
...
- `ci.yml`: runs on `push` and `pull_request` for `main`
- `deploy-pages.yml`: runs on `push` to `main` and deploys the static export to GitHub Pages
